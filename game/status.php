<?php
// --- All calculations to do with status
...
class Status
{
            ...
            switch ($status) {
                case 'PAR':
                    if (($pokemon['Type 1'] == 'Electric' || $pokemon ['Type 2'] == 'Electric') || (($pokemon['Type 1'] == 'Ground' || $pokemon ['Type 2'] == 'Ground') && $move == 'Thunder Wave')) {
                        ...
                        echo "\n " . $pokemon['Name'] . " became " . $this->loader->statusColor($pokemon['Status']) . "paralyzed\e[0m!\n";
                    }
                    break;
                case 'BRN':
                    if ($pokemon['Type 1'] == 'Fire' || $pokemon ['Type 2'] == 'Fire') {
                        ...
                        $pokemon['Status'] = 'BRN';
                        echo "\n " . $pokemon['Name'] . " was " . $this->loader->statusColor($pokemon['Status']) . "burned\e[0m!\n";
                    }
                    break;
                case 'FRZ':
                    if ($pokemon['Type 1'] == 'Ice' || $pokemon ['Type 2'] == 'Ice') {
                        ...
                        $pokemon['Status'] = 'FRZ';
                        echo "\n " . $pokemon['Name'] . " was " . $this->loader->statusColor($pokemon['Status']) . "frozen\e[0m solid!\n";
                    }
                    break;
                case 'PSN':
                    if ($pokemon['Type 1'] == 'Poison' || $pokemon ['Type 2'] == 'Poison' || $pokemon['Type 1'] == 'Steel' || $pokemon ['Type 2'] == 'Steel') {
                        ...
                        $pokemon['Status'] = 'PSN';
                        echo "\n " . $pokemon['Name'] . " was " . $this->loader->statusColor($pokemon['Status']) . "poisoned\e[0m!\n";
                    }
                    break;
                case 'SLP':
                    $pokemon['Status'] = 'SLP';
                    echo "\n " . $pokemon['Name'] . " fell " . $this->loader->statusColor($pokemon['Status']) . "asleep\e[0m!\n";
    ...
    public function paralyze(&$pokemon)
    {
        ...
        if ($parHit <= 25) {
            sleep(2);
            echo "\n " . $pokemon['Name'] . " is " . $this->loader->statusColor($pokemon['Status']) . "paralyzed\e[0m! It can't move!\n";
    ...
    {
        sleep(2);
        echo "\n " . $pokemon['Name'] . " is hurt by it's " . $this->loader->statusColor($pokemon['Status']) . "burn\e[0m!\n";
    ...
    public function freeze(&$pokemon)
    {
        ...
        if ($frzHit >= 20) {
            sleep(2);
            echo "\n " . $pokemon['Name'] . " is " . $this->loader->statusColor($pokemon['Status']) . "frozen\e[0m solid!\n";
            $pokemon['Move Turns'] = 0;
            return true;
        } else {
    ...
    {
        sleep(2);
        echo "\n " . $pokemon['Name'] . " is hurt by " . $this->loader->statusColor($pokemon['Status']) . "poison\e[0m!\n";
    ...
    public function sleep(&$pokemon)
    {
        if ($pokemon['Sleep'] > 0) {
            sleep(2);
            echo "\n " . $pokemon['Name'] . " is fast " . $this->loader->statusColor($pokemon['Status']) . "asleep\e[0m!\n";
            $pokemon['Move Turns'] = 0;
            $pokemon['Sleep']--;
            return true;
        } else {
    ...
    public function confusion(&$pokemon)
    {
        if ($pokemon['Confusion'] > 0) {
            ...
            if ($confuseHit <= 33) {
                ...
                $pokemon['Move Turns'] = 0;
                $damage = round((((((2 * 50) / 5) + 2) * (40 * ($this->stat->getStat($pokemon, 'Attack') / $this->stat->getStat($pokemon, 'Defence'))) / 50) + 2));
                $pokemon['HP Left'] = $pokemon['HP Left'] - $damage;
                echo "It deals " . $damage . " damage.\n";
                return true;
            } else {
